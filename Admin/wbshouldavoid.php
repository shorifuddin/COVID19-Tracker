<?php
		...
		if (!empty($_POST)) {
			...
			if ($img['name']!='') {
				$imgname='Prevention-Should-Avoid'. time().rand(10,1000).'.'.pathinfo($img['name'],PATHINFO_EXTENSION);
			}

			if (!empty($title)) {
				
				
				$inser="INSERT INTO preavoid(title,decp,img) 
						VALUE('$title','$decp','$imgname')";
					if (mysqli_query($con,$inser)) {
						move_uploaded_file($img['tmp_name'],'../upload/Prevention/'.$imgname);
						echo "Prevention-Should-Avoid ADD Successfully";
					}else{
						echo "Prevention-Should-Avoid ADD Failed";
					}
      ...
      <div class="card-header bg-secondary card_header">
          <div class="row">
            <div class="col-md-8 card_header_title">
              <i class="fab fa-gg-circle "></i>Prevention Should Avoid Update
            </div>
